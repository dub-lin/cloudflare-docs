...
## Exempt WARP traffic in Cloudflare WAF

Admins can add a custom header to an HTTP policy and the same custom header to a WAF firewall rule to exempt their certain users from their WAF.
...
2. In Cloudflare WAF, [create a custom rule](/waf/custom-rules/) with the same [custom header value](/waf/custom-rules/use-cases/require-specific-headers/).
