...
When the `isolatedStorage` flag is enabled (the default), the test runner will undo any writes to the storage at the end of the test as detailed in the [Isolation and concurrency docs](/workers/testing/vitest-integration/isolation-and-concurrency/). However, you must manage resources carefully. Below are some common pitfalls and how to avoid them:
...
When making requests via `fetch` or `R2.get()`, consume the entire response body, even if you aren't asserting its content. For example:
